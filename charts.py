...
import ta
from theme import ACCENT, BG, BORDER, CYAN, DOWN, GOLD, MUTED, PURPLE, TEXT, UP, VIOLET
...
def oi_by_strike(calls, puts, price, title="Open interest by strike", names=("Calls", "Puts")):
    fig = go.Figure()
    if not calls.empty:
        fig.add_trace(go.Bar(x=calls["strike"], y=calls["openInterest"].fillna(0), name=names[0], marker_color=UP, opacity=0.85))
    if not puts.empty:
        fig.add_trace(go.Bar(x=puts["strike"], y=-puts["openInterest"].fillna(0), name=names[1], marker_color=DOWN, opacity=0.85))
...
def payoff(kind, strike, premium, price_now, title="Profit / loss at expiration", labels=("Stock price at expiration", "Profit / loss ($)")):
    ...
    fig.update_xaxes(title_text=labels[0], showgrid=True, gridcolor=BORDER)
...
def movers_bubble(df, title="Change vs relative volume (bubble = market cap)", xlab="Relative volume (×)", ylab="Change %"):
    ...
    fig = go.Figure(go.Scatter(
        ...
        marker=dict(size=size, color=d["Chg %"], colorscale=[[0, DOWN], [0.5, "#2A3142"], [1, UP]], cmid=0,
                    line=dict(width=1, color=BG), opacity=0.9),
    ...
    fig.update_xaxes(title_text=xlab, type="log", showgrid=True, gridcolor=BORDER)
